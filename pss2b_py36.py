...
def  residuals_PSS_T(p):
    ...
    return  -90-AVR_phase-PSS_phase  #标幺化后进行的计算，否则误差会很大
...
#幅值算的有点不对，不知道原因是什么.
#原因找到了，在使用abs时，应该是np.abs(1+1j*Tw3*ws),而不是np.abs(1+Tw3*ws)
PSS_amplify1=np.exp(\
...
PSS_amplify2=np.abs(1j*Tw3*ws)/np.abs(1+1j*Tw3*ws) \
    *np.abs(Ks2)/np.abs(1+1j*T7*ws)\
    *np.abs(1+1j*T1*ws)/np.abs(1+1j*T2*ws)\
    *np.abs(1+1j*T3*ws)/np.abs(1+1j*T4*ws)
